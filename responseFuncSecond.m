function [displacementResponse, velocityResponse, accelerationResponse, totalResponse] = responseFuncSecond(m, M, r, c, k, F_init, forced_freq, x_cond, xdot_cond)
    ...
    nat_freq = sqrt(k/M);   % Natural frequency (rad/s)
    static_def = F_init/k;  % Static deflection (m)
    damping_ratio =c / (2 * sqrt(k * M)); % Damping ratio
    X_amplitude = (F_init / sqrt((k - M * forced_freq^2)^2 + (c*forced_freq)^2));
    %X_amplitude = 1.387E-03;
    r_ratio = forced_freq / nat_freq;

    % Calculate phase angle
    if c == 0
        phase = 0;
    else
        phase = atan((2 * damping_ratio * r_ratio) / (1 - r_ratio^2));
    ...
    if (c == 0) && (damping_ratio == 0)
        ...
        X_amplitude = (F_init / (k - m * forced_freq^2));
        x_particular = X_amplitude * cos(forced_freq * t);
    ...
    elseif damping_ratio < 1
        ...
        comput = -damping_ratio * nat_freq;
        %fprintf('comput: %.3f kg |\t', comput);
        x_homogeneous = exp(-damping_ratio * nat_freq * t) *(U * cos(damp_freq * t) + V * sin(damp_freq * t));
        x_particular = X_amplitude * cos(forced_freq * t - phase);

    elseif damping_ratio > 1
        disp("Overdamped system (ζ > 1)")
        s1 = -damping_ratio * nat_freq - nat_freq * sqrt(damping_ratio^2 - 1);
        s2 = -damping_ratio * nat_freq + nat_freq * sqrt(damping_ratio^2 - 1);
    ...
    x_total = vpa(subs(x_total, [U, V], [total_solution.U, total_solution.V]),5);
    ...
    fprintf('(ζ): %.3f\n', damping_ratio);
    fprintf('X: %.5f m\n', X_amplitude);
    fprintf('r_ratio: %.3f m\n', r_ratio);
    fprintf('(ϕ): %.3f rad\n', phase);

    if damping_ratio < 1
        damp_freq = nat_freq * sqrt(1 - damping_ratio^2); % Recalculate if needed
        fprintf('(ω_d): %.10f rad/s\n', damp_freq);
    end
    
    if damping_ratio > 1
        fprintf('S1: %.3f |\t', vpa(s1, 3));
        fprintf('S2: %.3f |\n', vpa(s2, 3));
    end
